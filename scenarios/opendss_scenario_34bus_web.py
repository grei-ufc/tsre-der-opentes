"""IEEE34 com regulação de tensão em malha fechada e visualização no navegador.

O mesmo cenário de [opendss_scenario_34bus.py], acrescido da visualização web:
as barras são desenhadas nas coordenadas reais do alimentador e coloridas pela
tensão, com os botões ``A``/``B``/``C``/``mín``/``máx``/``méd``/``desb`` no
canto superior direito trocando qual grandeza colore o mapa. Clicar num nó abre
...
# qualquer atraso positivo, e o atraso inicial (compilar o circuito) nunca é
# recuperado. Com vários simuladores isso enche o terminal sem indicar problema
# algum — a visualização continua em dia. Um atraso que importe aparece como
# lentidão visível no navegador.
...
# O .dss principal não carrega as coordenadas; sem isto o desenho cai no layout
# de forças.
BUSCOORDS = DATA_DIR / "IEEE34_BusXY.csv"
...
# Um passo de 5 min a cada 2 s de relógio: o dia inteiro leva ~10 min. Use None
# para rodar o mais rápido possível.
RT_FACTOR = 1 / 1000
# RT_FACTOR = None
