...
from ._utils import to_3phase, extract_3phase_pq
...
class OpenDSSSimulator(mosaik_api_v3.Simulator):
    ...
    def get_data(self, outputs):
        ...
        for eid, attrs in outputs.items():
            ...
            if model_type == 'Load':
                ...
                data[eid].update(
                    self._extract_line_data(name, attrs)
                )

            elif model_type == 'Bus':
                data[eid].update(
                    self._extract_bus_data(name, attrs)
                )
                ...
                data[eid].update(
                    extract_3phase_pq(
                        self.dss_wrapper, name, 'Storage', attrs, sign=-1,
                    )
                )
                ...
                data[eid].update(
                    extract_3phase_pq(
                        self.dss_wrapper, name, 'PVSystem', attrs, sign=-1,
                    )
                )
    ...
    def _extract_line_data(self, name, attrs):
        ...
        curr_mag, curr_ang = self.dss_wrapper.get_current(
            name, element='Line', polar=True, mag_only=False, line_bus=1,
        )
        mags = to_3phase(curr_mag)
        angs = to_3phase(curr_ang)

        current_map = {
            'I1_A': mags[0], 'I2_A': mags[1], 'I3_A': mags[2],
        ...
        p_w, q_var = self.dss_wrapper.get_power(
            name, element='Line', line_bus=1,
        )
        ps = to_3phase(p_w)
        qs = to_3phase(q_var)

        power_map = {
            'P1_w': ps[0], 'P2_w': ps[1], 'P3_w': ps[2],
    ...
    def _extract_bus_data(self, name, attrs):
        ...
        volt_mag, volt_ang = self.dss_wrapper.get_bus_voltage(
            bus=name, pu=True, mag_only=False, polar=True,
        )
        mags = to_3phase(volt_mag)
        angs = to_3phase(volt_ang)

        voltage_map = {
            'V1_pu': mags[0], 'V2_pu': mags[1], 'V3_pu': mags[2],
