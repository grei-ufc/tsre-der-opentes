...
from ..opendss import topology_builder as tb  # Topology building and bus classification
from . import pv_validator as val  # Centralised validation and error-checking routines
...
class PVGenerator:
    ...
    def __init__(
        ...
        # KVA Setup: snap the capacity to the nearest commercially available
        # inverter rating, chosen by phase count and voltage level (BT/MT).
        inversores_1f_BT = pd.Series([1, 1.5, 2, 2.5, 3, 3.6, 4, 5, 6, 7, 7.5, 8, 9, 10])
        inversores_3f_BT = pd.Series(
            [4, 5, 6, 8, 10, 12, 15, 17, 20, 25, 27, 30, 33, 36, 40, 45, 50, 60, 75]
        )
        inversores_3f_MT = pd.Series(
            [12, 15, 17, 20, 25, 27, 30, 33, 36, 40, 50, 75, 100, 110, 125, 150, 220, 250, 330, 350]
        )
        ...
        if pd.isna(PV_kva):
            match self.phases:
                ...
                case _:
                    raise ValueError(
                        f"Número de fases inválido ({self.phases}) para o PV {self.name}. Deve ser 1 ou 3."
                    )
        else:
            match self.phases:
                case 1 if self.kv <= 1.0:
                    valor_comercial = inversores_1f_BT.iloc[
                        (inversores_1f_BT - PV_kva).abs().idxmin()
                    ]
                ...
                case 1 if self.kv > 1.0:
                    ...
                    self.kva = valor_comercial
                case 3 if self.kv <= 1.0:
                    valor_comercial = inversores_3f_BT.iloc[
                        (inversores_3f_BT - PV_kva).abs().idxmin()
                    ]
                    self.kva = valor_comercial
                case 3 if self.kv > 1.0:
                    valor_comercial = inversores_3f_MT.iloc[
                        (inversores_3f_MT - PV_kva).abs().idxmin()
                    ]
                    self.kva = valor_comercial
                case _:
                    raise ValueError(
        ...
        self.irrad_curve = (
            self.solar_station_curves["poa_irradiance_wm2"].iloc[data_slice] / self.irrad
        ...
        self.temperature_curve = (
            self.solar_station_curves["panel_temperature_celsius"].iloc[data_slice]
            / self.temperature
        ).reset_index(drop=True)
        ...
        self.datetime = pd.to_datetime(
            self.solar_station_curves["datetime"].iloc[data_slice]
        ).reset_index(drop=True)
    ...
    def CurveLinearInterpolation(
        self, new_rate=step, npts_base_15min=npts_origin, start_date=start_date
    ):
        """
        ...
        self.irrad_curve = (
            self.irrad_curve.resample(new_rate).mean().interpolate(method="time")
        ).round(6)
        self.temperature_curve = (
            self.temperature_curve.resample(new_rate).mean().interpolate(method="time")
        ).round(6)
        ...
        self.temperature_curve = self.temperature_curve.reset_index().rename(
            columns={"index": "Date"}
        )
    ...
    def GenerateCSV(PVGen, OUTPUT_DIR=OUTPUT_DIR):
        ...
        irrad_list = [PVGen[0].irrad_curve] + [pv.irrad_curve.iloc[:, 1] for pv in PVGen[1:]]
    ...
    try:
        ...
        if PV_dictionaries_list is None:
            ...
            if not bus_multi_PV:
                ...
                Existent_PV_buses = pd.DataFrame(
                    {"bus": [dictionary["PV_bus"] for dictionary in PV_Dictionaries]}
                )
            else:
                Existent_PV_buses = pd.DataFrame(columns=["bus"])
        ...
        attachments = tb.collect_attachments(dss)

        # We exclude reference and regulator buses from being selected for PV installation.
        excluded_node_types = {"refbus", "regulator_bus"}
        ...
        for i in dss.circuit.buses_names:
            ...
            node_type = tb.get_node_type(dss.bus.name.lower(), attachments)

            if node_type not in excluded_node_types:
                # Define connection and phase count based on bus topology (nodes)
                allbuses_mapping.append(f"{dss.bus.name}." + ".".join(map(str, dss.bus.nodes)))
        ...
        if ignore_buses is not None:
            ...
            logger.info(
                f"Applied filter - Excluded {len(ignore_buses)} bus(es) from the candidate pool."
            )

        # Perform unique bus filtering if multi-PV is disabled
        available_buses = (
            allbuses[~allbuses["bus"].isin(Existent_PV_buses["bus"])]
            if not bus_multi_PV
            else allbuses
        )
        ...
        if PV_Dictionaries:
            ...
            logger.info(
                "No manual PV configurations detected. Proceeding with random allocation only."
            )

        logger.info(f"Randomly selected {QtdPVs} buses for PV installation.")

        logger.info(
            f"{((QtdPVs + len(PV_Dictionaries)) / len(available_buses)) * 100:.2f}% of available buses allocated for PV systems."
        )
        logger.info(
            f"{(QtdPVs + len(PV_Dictionaries))} buses occupied out of {len(available_buses)} total available buses."
        )
            ...
            PV_Dictionaries.append(
                {
                    ...
                    "PV_kv": float(
                        (PVbuses.loc[bus, "L-N kv"])
                        if (PVbuses.loc[bus, "phases"]) == 1
                        else round(((PVbuses.loc[bus, "L-N kv"]) * (3 ** (1 / 2))), 2)
                    ),
                    "PV_kva": None,
                    "PV_curve_id": None,
                }
        ...
        for PV, PV_data in enumerate(PV_Dictionaries):
            ...
            PVGen.append(new_pv)
            logger.info(
                f"   > {new_pv.name} configured at bus {new_pv.bus} ({new_pv.kva} kVA)"
            )
        print("")
