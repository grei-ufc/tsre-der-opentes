...
class VR_Model(object):
    ...
    def __init__(self, name, Ts, Td_ctrl=30, Td_tap=2, Vref=120, db=2, 
                 PT_Ratio=120, tap_max=16, tap_min=-16, tap_ini=0, **kwargs):
        self.name = name
        self.Ts = Ts
    ...
    def run(self, V_meas):
        ...
        # Lógica de Histerese (Cópia fiel da lógica fornecida)
        ...
        # Verifica se o tempo de atraso de controle expirou
        if self.Ti_ctrl > self.Td_ctrl:
            # Verifica se o tempo mecânico entre taps expirou
...
META = {
    ...
    'models': {
        'RegController': {
            'public': True,
            'params': ['vreg', 'band', 'pt_ratio', 'delay', 'tap_delay', 'tap_ini'],
            'attrs': ['v_meas', 'tap_cmd'], # v_meas (entrada), tap_cmd (saída)
        },
    },
}
