...
print('\nDiferença média da potência gerada do simulador em relação ao OpenDSS:', f"{dif_media_P:.3f}", 'W')
print('A maior diferença da potência gerada é:', f"{maior_dif_P:.3f}",'W')

#V1:
df['Dif_V1'] = abs((df_2['DSS-0.Bus-671-V1_pu']-df[' V1']))*2400
dif_total=0
maior_dif=0
j=0
for i, valor in enumerate(df['Dif_V1']):
    if df.at[i, 'Dif_V1'] < 70:
        dif_total=dif_total+df.at[i, 'Dif_V1']
        j=j+1
        if abs(df.at[i, 'Dif_V1']) > abs(maior_dif):
            maior_dif = df.at[i, 'Dif_V1']
maior_dif_V1 = maior_dif
maior_dif_V1_pu = maior_dif_V1/2400
dif_media_V1 = dif_total/j
dif_media_V1_pu = dif_media_V1/2400
print('\nDiferença média da tensão V1 do simulador em relação ao OpenDSS:', f"{dif_media_V1:.3f}",'V','=',f"{dif_media_V1_pu:.3f}",'pu')
print('A maior diferença de tensão para V1 é:',f"{maior_dif_V1:.3f}",'V','=',f"{maior_dif_V1_pu:.3f}",'pu')

#V2:
df['Dif_V2'] = abs((df_2['DSS-0.Bus-671-V2_pu']-df[' V2']))*2400
dif_total=0
maior_dif=0
j=0
for i, valor in enumerate(df['Dif_V2']):
    if df.at[i, 'Dif_V2'] < 70:
        dif_total=dif_total+df.at[i, 'Dif_V2']
        j=j+1
        if abs(df.at[i, 'Dif_V2']) > abs(maior_dif):
            maior_dif = df.at[i, 'Dif_V2']
maior_dif_V2 = maior_dif
maior_dif_V2_pu = maior_dif_V2/2400
dif_media_V2 = dif_total/j
dif_media_V2_pu = dif_media_V2/2400
print('\nDiferença média da tensão V2 do simulador em relação ao OpenDSS:', f"{dif_media_V2:.3f}",'V','=',f"{dif_media_V2_pu:.3f}",'pu')
print('A maior diferença de tensão para V2 é:',f"{maior_dif_V2:.3f}",'V','=',f"{maior_dif_V2_pu:.3f}",'pu')

#V3:
df['Dif_V3'] = abs((df_2['DSS-0.Bus-671-V3_pu']-df[' V3']))*2400
dif_total=0
maior_dif=0
j=0
for i, valor in enumerate(df['Dif_V3']):
    if df.at[i, 'Dif_V3'] < 70:
        dif_total=dif_total+df.at[i, 'Dif_V3']
        j=j+1
        if abs(df.at[i, 'Dif_V3']) > abs(maior_dif):
            maior_dif = df.at[i, 'Dif_V3']
maior_dif_V3 = maior_dif
maior_dif_V3_pu = maior_dif_V3/2400
dif_media_V3 = dif_total/j
dif_media_V3_pu = dif_media_V3/2400
print('\nDiferença média da tensão V3 do simulador em relação ao OpenDSS:', f"{dif_media_V3:.3f}",'V','=',f"{dif_media_V3_pu:.3f}",'pu')
print('A maior diferença de tensão para V3 é:',f"{maior_dif_V3:.3f}",'V','=',f"{maior_dif_V3_pu:.3f}",'pu')
print('')
