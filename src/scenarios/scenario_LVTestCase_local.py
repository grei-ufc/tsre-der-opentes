    ...
    with mosaik.World(SIM_CONFIG) as world:
        ...
        for info in pv_info:
            ...
            if eid_dss in pvs_dss_map:
                ...
                cols_irr = [c for c in pd.read_csv(DATA_DIR_HOST / "LVTestCase_shape_pv_5min.csv", nrows=0).columns if c.lower() != 'time']
                cols_tmp = [c for c in pd.read_csv(DATA_DIR_HOST / "LVTestCase_temperature_5min.csv", nrows=0).columns if c.lower() != 'time']
